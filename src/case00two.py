...
for _ in tqdm(range(3000)):
    ...
    g_t = np.random.normal(size=n_samples)
    alphas = np.random.uniform(low=0.1, high=1.0, size=N)
    yitas = np.random.uniform(low=0.1, high=1.0, size=N)
    u_it = np.random.normal(size=(n_samples, N))
    correlated_errors = f_t[:, np.newaxis] * alphas[np.newaxis,:] + g_t[:, np.newaxis] * yitas[np.newaxis,:] + u_it
    series_list=[]
...
with open('case00two.txt', 'a+') as f:
...
for k in tqdm(range(3000)):
    ...
    f_t1 = np.random.normal(size=int(1.1 * m))
    g_t1 = np.random.normal(size=int(1.1 * m))
    alphas1 = np.array([0.2711117, 0.92084525])
    yitas1 = np.array([0.6111117, 0.32084525])
    u_it1 = np.random.normal(size=(int(1.1 * m), N))
    correlated_errors1 = f_t1[:, np.newaxis] * alphas1[np.newaxis, :] + g_t1[:, np.newaxis] * yitas1[np.newaxis,:] + u_it1
    f_t2 = np.random.normal(size=n_samples - int(1.1 * m))
    g_t2 = np.random.normal(size=n_samples - int(1.1 * m))
    alphas2 = np.array([1, 0])
    yitas1 = np.array([0.6111117, 0.32084525])
    u_it2 = np.random.normal(size=(n_samples - int(1.1 * m), N))
    correlated_errors2 = f_t2[:, np.newaxis] * alphas2[np.newaxis, :] + g_t2[:, np.newaxis] * yitas1[np.newaxis,:] + u_it2
