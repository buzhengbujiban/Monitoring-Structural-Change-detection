...
q = args.q
n_samples = q * m + 5
selected_lag_order = 2 ### depending on your test
...
for _ in tqdm(range(3000)):
    ...
    for i in range(N):
        arma_model = sm.tsa.ArmaProcess(ar=np.r_[2, -ar_param], ma=np.r_[2, ma_param])  ### depending on your case
...
with open('case3.txt', 'a+') as f:    ### depending on your case
...
for k in tqdm(range(3000)):
    ...
    for i in range(N):
        arma_model = sm.tsa.ArmaProcess(ar=np.r_[2, -ar_param], ma=np.r_[2, ma_param])
        series = arma_model.generate_sample(nsample=n_samples, scale=1,
                                            distrvs=lambda size: correlated_errors[:size[0], i])
        series_list.append(series)
    df = np.array(series_list, dtype=np.float32).T
