...
for _ in tqdm(range(3000)):
    ...
    f_t = np.random.laplace(0, 1, n_samples)   ### depending on your case
    alphas = np.random.uniform(low=0.1, high=1.0, size=N)
    u_it = np.random.laplace(0, 1, (n_samples, N)) ### depending on your case
...
with open('case5.txt', 'a+') as f:    ### depending on your case
...
q = args.q
n_samples = q * m + 5
selected_lag_order = 5  ### depending on your test
...
for k in tqdm(range(3000)):
    ...
    f_t1 = np.random.laplace(0, 1, int(1.1*m)) ### depending on your case
    alphas1 = np.array([0.2711117, 0.92084525])
    u_it1 = np.random.laplace(0, 1, (int(1.1*m), N)) ### depending on your case
    correlated_errors1 = f_t1[:, np.newaxis] * alphas1[np.newaxis, :] + u_it1

    f_t2 = np.random.laplace(0, 1, n_samples - int(1.1*m))  ### depending on your case
    alphas2 = np.array([1, 0])
    u_it2 = np.random.laplace(0, 1, (n_samples - int(1.1*m), N)) ### depending on your case
