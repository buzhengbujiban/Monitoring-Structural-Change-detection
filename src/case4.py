...
q = args.q
n_samples = q * m + 5
selected_lag_order = 4 ### depending on your test
...
for _ in tqdm(range(3000)):
    ...
    f_t = generate_garch11(n_samples, omega, alpha, beta)    ### depending on your case
    alphas = np.random.uniform(low=0.1, high=1.0, size=N)
    u_it =np.stack((generate_garch11(n_samples, omega, alpha, beta), generate_garch11(n_samples, omega, alpha, beta)), axis=-1)   ### depending on your case
    correlated_errors = f_t[:, np.newaxis] * alphas[np.newaxis, :] + u_it
    series_list = []

    for i in range(N):
        arma_model = sm.tsa.ArmaProcess(ar=np.r_[1, -ar_param], ma=np.r_[1, ma_param])  ### depending on your case
...
with open('case4.txt', 'a+') as f:    ### depending on your case
...
for k in tqdm(range(3000)):
    ...
    f_t1 = generate_garch11(int(1.1*m), omega, alpha, beta) ### depending on your case
    alphas1 = np.array([0.2711117, 0.92084525])
    u_it1 = np.stack((generate_garch11(int(1.1*m), omega, alpha, beta), generate_garch11(int(1.1*m), omega, alpha, beta)), axis=-1)  ### depending on your case
    correlated_errors1 = f_t1[:, np.newaxis] * alphas1[np.newaxis, :] + u_it1

    f_t2 = generate_garch11(n_samples - int(1.1*m), omega, alpha, beta)  ### depending on your case
    alphas2 = np.array([1, 0])
    u_it2 = np.stack((generate_garch11(n_samples - int(1.1*m), omega, alpha, beta), generate_garch11(n_samples - int(1.1*m), omega, alpha, beta)), axis=-1) ### depending on your case
