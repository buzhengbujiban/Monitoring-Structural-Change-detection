...
for _ in tqdm(range(3000)):
    ...
    f_t = generate_garch11(n_samples, omega, alpha, beta)
    alphas = np.random.uniform(low=0.1, high=1.0, size=N)
    u_it = np.stack((generate_garch11(n_samples, omega, alpha, beta), generate_garch11(n_samples, omega, alpha, beta)),
                    axis=-1)
...
for _ in tqdm(range(3000)):
    ...
    f_t1 = generate_garch11(int(1.1 * m), omega, alpha, beta)
    alphas1 = np.array([0.2711117, 0.92084525])
    u_it1 = np.stack(
        (generate_garch11(int(1.1 * m), omega, alpha, beta), generate_garch11(int(1.1 * m), omega, alpha, beta)),
        axis=-1)
    correlated_errors1 = f_t1[:, np.newaxis] * alphas1[np.newaxis, :] + u_it1

    f_t2 = generate_garch11(n_samples - int(1.1 * m), omega, alpha, beta)
    alphas2 = np.array([1, 0])
    u_it2 = np.stack((generate_garch11(n_samples - int(1.1 * m), omega, alpha, beta),
                      generate_garch11(n_samples - int(1.1 * m), omega, alpha, beta)), axis=-1)
