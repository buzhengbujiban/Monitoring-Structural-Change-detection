...
## update way
residuals_list = np.full((3000, n_samples-3, N), np.nan, dtype=np.float32)
for kk in tqdm(range(3000)):
...
for serx in range(N-1):
    for sery in range(serx, N):
        ...
        print('size test, countfail={}, m={}, q={}, a={}, serx={}, sery={}'.format(countfail, m, q, a, serx, sery))
        with open('case0.txt', 'a+') as f:
            f.writelines('size test, countfail={}, m={}, q={}, a={}, serx={}, sery={}\n'.format(countfail, m, q, a, serx, sery))
...
residuals_list_power = np.full((3000, n_samples-3, N), np.nan, dtype=np.float32)
for kk in tqdm(range(3000)):

    f_t1 = np.random.normal(size=int(1.1*m))  ### depending on your case
    alphas1 = np.array([0.2711117, 0.92084525, 0.30668, 0.72536, 0.59126])[:N]
    ...
    alphas2 = np.array([1, 0, 0.5, 0.72536, 0.59126])[:N]
