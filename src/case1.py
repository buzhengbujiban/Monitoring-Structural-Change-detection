...
for _ in tqdm(range(3000)):
    ...
    f_t = t.rvs(df=20, size=n_samples)
    alphas = np.random.uniform(low=0.1, high=1.0, size=N)
    u_it = t.rvs(df=20, size=(n_samples, N))
...
for k in tqdm(range(3000)):
    ...
    f_t1 = t.rvs(df=20, size=int(1.1 * m))
    alphas1 = np.array([0.2711117, 0.92084525])
    u_it1 = t.rvs(df=20, size=(int(1.1 * m), N))
    correlated_errors1 = f_t1[:, np.newaxis] * alphas1[np.newaxis, :] + u_it1

    f_t2 = t.rvs(df=20, size=(n_samples - int(1.1 * m)))
    alphas2 = np.array([1, 0])
    u_it2 = t.rvs(df=20, size=(n_samples - int(1.1 * m), N))
